...
}}}}return b}.property("anchorLocation").cacheable()});Greenhouse.libraryController=SC.TreeController.create(SC.CollectionViewDelegate,{reload:function(){var d=Greenhouse.CONFIG_QUERY,c=Greenhouse.targetController.get("content");
...
}},this);return a}.property("[]").cacheable()});Greenhouse.viewConfigsController=SC.ArrayController.create({reload:function(){var c=Greenhouse.CONFIG_QUERY,b=Greenhouse.targetController.get("content");
c.set("app",b.get("name"));var a=Greenhouse.store.find(c);this.set("content",a)},_content_status_changed:function(){var b=this.get("content"),a=this;
if(b&&b.get&&b.get("status")&&b.get("status")===SC.Record.READY_CLEAN){Greenhouse.libraryController.set("content",SC.Object.create({treeItemIsExpanded:YES,treeItemChildren:[SC.Object.create({name:"Views",treeItemIsExpanded:YES,treeItemChildren:a.get("views")}),SC.Object.create({name:"Controllers",treeItemIsExpanded:YES,treeItemChildren:a.get("controllers")}),SC.Object.create({name:"Panes",treeItemIsExpanded:YES,treeItemChildren:a.get("panes")})]}))
...
}.property("[]").cacheable(),_collect_all_the_elements:function(b){var e=this.get("content"),a=[],d;
if(e&&e.get("length")>0){e.forEach(function(c){d=c.get(b);if(d){d.forEach(function(f){a.pushObject(f)
})}})}return a},editable:function(){var a=[],b=this.get("content");if(b){b.forEach(function(c){if(c.get("canEdit")===YES){a.pushObject(c)
}})}return a}.property("content").cacheable()});require("core");Greenhouse.File=SC.ChildRecord.extend({type:"File",childRecordNamespace:Greenhouse,name:SC.Record.attr(String),dir:SC.Record.attr(String),body:SC.Record.attr(String),primaryKey:"id",isFile:YES,path:function(){return this.get("dir")+this.get("name")
...
a.set("contentView",Greenhouse.dialogPage.get("customViewModal"));a.set("layout",{centerX:0,centerY:0,width:350,height:380});
Greenhouse.set("newDesignClass",null);Greenhouse.set("newDesignDefaults",null);Greenhouse.set("newDesignViewConfig",null);
Greenhouse.set("newDesignType",null);a.append()},exitState:function(){var a=Greenhouse.dialogPage.get("modal");
a.remove();Greenhouse.set("newDesignClass",null);Greenhouse.set("newDesignDefaults",null);
Greenhouse.set("newDesignViewConfig",null);Greenhouse.set("newDesignType",null)},cancel:function(){this.goState("modalReady")
...
Greenhouse.viewConfigsController.refreshContent();this.goState("modalReady")}}),newPage:SC.State.create({parentState:"projectPicker",parallelStatechart:"modals",enterState:function(){var a=Greenhouse.dialogPage.get("modal");
...
Greenhouse.viewConfigsController.reload();this.goState("loading")}}),iframeLoading:SC.State.create({enterState:function(){var a=Greenhouse.getPath("mainPage.mainPane.container");
